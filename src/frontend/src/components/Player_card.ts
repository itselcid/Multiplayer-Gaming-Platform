...
export class Player_card extends Component {
	...
	render(): void {
		...
		const pfp = addElement('div', 'w-24 h-24 mx-auto mb-6 rounded-full bg-gradient-to-br from-neon-cyan to-neon-purple flex items-center justify-center text-4xl overflow-hidden', this.el);

		// Only show avatar image for logged-in users
		if (user && user.avatar) {
			const avatarPath = user.avatar;
			const avatarUrl = avatarPath.startsWith('http') ? avatarPath : `/public${avatarPath}`;
			pfp.innerHTML = `<img src="${avatarUrl}" alt="${displayName}" class="w-full h-full object-cover" onerror="this.style.display='none'; this.parentElement.innerHTML='${displayInitial}';" />`;
		} else {
			// Guest or no avatar - show initial
			pfp.textContent = displayInitial;
		}
