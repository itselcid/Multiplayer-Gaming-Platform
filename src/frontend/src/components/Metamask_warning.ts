...
export class Metamask_network_warning extends Component {
	...
	render(): void {
		this.el.id = 'metamask-network-warning';
...
export class Metamask_account_warning extends Component {
	private	_current_account: string;
	private	_user_account: string;

	constructor(current_account: string, user_account: string) {
		super('div', 'fixed inset-0 z-10 bg-black/40 backdrop-blur-sm overflow-y-auto overscroll-y-auto no-scrollbar flex justify-center items-start');
		this._current_account = current_account;
		this._user_account = user_account;
	}

	render(): void {
		this.el.id = 'metamask-account-warning';
		...
		container3.insertAdjacentHTML('beforeend', `
				...
				<p class="text-slate-300 text-base leading-relaxed max-w-md mx-auto">
					You're connected to the wrong account.
				</p>
				</div>
			`);

		container3.insertAdjacentHTML('beforeend', `
				<div class="bg-yellow-950/30 rounded-xl p-5 border border-yellow-500/30">
				<div class="flex items-start space-x-4">
					<div class="w-8 h-8 rounded-lg bg-yellow-500/20 flex items-center justify-center flex-shrink-0">
					<svg class="w-5 h-5 text-yellow-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
						<path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
					...
					<div class="flex-1">
					<h3 class="text-yellow-300 font-semibold text-sm mb-1.5">Account Mismatch</h3>
					<p class="text-slate-400 text-xs leading-relaxed mb-3">
						Registered users must connect using their linked MetaMask account.
					</p>
					<div class="flex items-center space-x-2">
						<div class="flex-1 bg-slate-800/50 rounded px-3 py-2 border border-slate-700">
						<p class="text-slate-500 text-xs">Current Account</p>
						<p class="text-yellow-400 text-sm font-mono">${shortenEthAddress(this._current_account)}</p>
						</div>
						<svg class="w-4 h-4 text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
						<path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" />
						</svg>
						<div class="flex-1 bg-slate-800/50 rounded px-3 py-2 border border-green-500/30">
						<p class="text-slate-500 text-xs">Required Account</p>
						<p class="text-green-400 text-sm font-mono">${shortenEthAddress(this._user_account)}</p>
						</div>
					</div>
					</div>
				</div>
